...
class PredictDoFn(beam.DoFn):
    ...
    def process(self, metric_json_val):
        ...
        for key in metric_json_val.keys():
            if (metric_json_val[key]) is None:
                return Null
...
def run(argv=None):
  ...
  with beam.Pipeline(options=pipeline_options) as p:
    ...
    images= (p | "Read from Pub/Sub" >> beam.io.ReadFromPubSub(topic=known_args.input)| "toDict" >> beam.Map(lambda x: json.loads(x)));
